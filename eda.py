...
def plot_lot_hour_heatmap(df: pd.DataFrame, out_dir) -> None:
    """Künye 9.2: çoklu otopark × saat ortalama doluluk heatmap."""
    if "SystemCodeNumber" not in df.columns:
        return
    d = df.copy()
    d["hour"] = d["LastUpdated"].dt.hour.astype(int)
    d["lot_short"] = d["SystemCodeNumber"].astype(str).str[-12:]
    piv = (
        d.groupby(["lot_short", "hour"], observed=False)["occupancy_rate"]
        .mean()
    ...
    if piv.empty:
        return
    piv = piv.sort_index()
    piv = piv.reindex(columns=list(range(24)), fill_value=np.nan)
    h = max(4.0, min(24.0, 0.22 * len(piv)))
    ...
    plt.colorbar(im, label="Ortalama doluluk oranı")
    plt.yticks(range(len(piv.index)), piv.index, fontsize=7)
    plt.xticks(range(0, 24, 2), [str(x) for x in range(0, 24, 2)])
    plt.xlabel("Saat")
    plt.ylabel("Otopark (SystemCodeNumber son 12 karakter)")
    plt.title("Lot × saat doluluk heatmap (Birmingham)")
    plt.tight_layout()
    plt.savefig(out_dir / "8_lot_saat_heatmap.png", dpi=150, bbox_inches="tight")
    plt.close()
